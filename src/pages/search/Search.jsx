...
export default function Search() {
  ...
  useEffect(() => {
    ...
    if (query && currentPage) {
      ...
      instanceTmdb
        .get(
          `/search/multi?query=${query}&include_adult=false&language=${i18n.language}&page=${currentPage}`
        )
        .then(({ data }) => {
          ...
          <ul className="lg:grid lg:w-2/3 lg:grid-cols-2">
            {results
              ?.filter((data) => {
                ...
                } else return data.profile_path;
              })
              .map((data, index) => (
                <ResultCard data={data} index={index} key={data.id} />
              ))}
          </ul>
          <ReactPaginate
